...
public class AppiumAppElement extends AppiumWrapperBase implements AppiumMobileElement {
	protected MobileElement mobileElement;
	protected List<MobileElement> mobileElements;

	public AppiumAppElement(AppiumMobileDriver appiumMobileDriver) {
		AppiumWrapperBase.appiumMobileDriver = appiumMobileDriver;
		reportManager = appiumMobileDriver.getReportManager();
	}

	public AppiumAppElement as(String as) {
		this.alias = "'" + as + "',";
		AppiumDriverBase.alias = this.alias;
		this.separator = " ";
		return this;
	}
	...
	public void setWebElements(By locator) {
		...
		this.mobileElements = appiumMobileDriver.getMobileDriver().findElements(locator);
		if (this.mobileElements.size() == 0) {
//			System.out.println("!!!!!!!!!! No element found on the screen for locator "+locator+" !!!!!!!!!!");
	...
	public void setWebElements(MobileElement element, String locator) {
		...
		this.mobileElements = new ArrayList<MobileElement>();
	...
	public List<AppiumMobileElement> getAllMobileElements() {
		...
		for (MobileElement androidElement : this.mobileElements) {
			AppiumAppElement genericMobileElement = new AppiumAppElement(appiumMobileDriver);
	...
	public void click() {
		...
		String logMessage = "Clicked on " + alias + separator + this.locator;
	...
	public AppiumMobileElement scrollDownUntilVisible() {
		JavascriptExecutor js = appiumMobileDriver.getMobileDriver();
		...
		while (!this.mobileElement.isDisplayed()) {
			if (counter >= 0) {
				...
				System.out.println("Expected element is not visible of the screen, attribute value is "
						+ this.mobileElement.isDisplayed());
			}
		}
		reportManager.reportPass("scrollDownUntillVisible " + alias + separator + this.locator);
		System.out.println("scrollDownUntillVisible " + alias + separator + this.locator);
		return this;
	}
	...
	public void setValue(String value) {
		setStepNameDefault();
		this.mobileElement.click();
		this.mobileElement.setValue("0.0");
	...
	public void setValue(float value) {
		...
		this.mobileElement.click();
		this.mobileElement.setValue("0.0");
		this.mobileElement.setValue(Float.toString(value));
		reportManager.reportPass("Entered \"" + value + "\" in " + alias + " " + this.locator);
		System.out.println("Entered \"" + value + "\" in " + alias + " " + this.locator);
	}
	...
	public String getText() {
		...
		reportManager.reportPass("Retrieved text \"" + returnText + "\" from " + alias + " " + this.locator);
	...
	public String getAttribute(String attr) {
		...
		reportManager.reportPass("Retrieved attribute \"" + returnText + "\" from " + alias + " " + this.locator);
		System.out.println("Retrieved \"" + returnText + "\" from " + alias + " " + this.locator);
		return returnText;
	}
	...
	public String getCssValue(String attr) {
		setStepNameDefault();
		String returnText = this.mobileElement.getCssValue(attr);
		reportManager.reportPass("Retrieved css value \"" + returnText + "\" from " + alias + " " + this.locator);
		System.out.println("Retrieved \"" + returnText + "\" from " + alias + " " + this.locator);
		return returnText;
	}
	...
	public String getTagName() {
		setStepNameDefault();
		String returnText = this.mobileElement.getTagName();
		reportManager.reportPass("Retrieved tag name \"" + returnText + "\" from " + alias + " " + this.locator);
		System.out.println("Retrieved \"" + returnText + "\" from " + alias + " " + this.locator);
		return returnText;
	}
	...
	public boolean waitTillVisible() {
		setStepNameDefault();
		reportManager.addScreenCapture(appiumMobileDriver.getMobileDriver());
		boolean returnText = this.mobileElement.isDisplayed();
		int counter = 20;
		while (returnText) {
			try {
				Thread.sleep(500);
	...
	public boolean isEnabled() {
		setStepNameDefault();
		reportManager.addScreenCapture(appiumMobileDriver.getMobileDriver());
		boolean returnText = this.mobileElement.isEnabled();
		String logMessage = "Verified isEnabled \"" + returnText + "\" from " + alias + " " + this.locator;
		if (returnText) {
			reportManager.reportPass(logMessage);
			System.out.println(logMessage);
		} else {
	...
	public boolean isSelected() {
		setStepNameDefault();
		reportManager.addScreenCapture(appiumMobileDriver.getMobileDriver());
		boolean returnText = this.mobileElement.isSelected();
		String logMessage = "Verified isSelected \"" + returnText + "\" from " + alias + " " + this.locator;
		if (returnText) {
			reportManager.reportPass(logMessage);
			System.out.println(logMessage);
		} else {
	...
	public String clear() {
		setStepNameDefault();
		String returnText = this.mobileElement.getText();
		mobileElement.clear();
	...
	public boolean verifyAllEqualsTo(DataTable expected) {
		...
		for (int i = 0; i < data.size(); i++) {
			pobQuestions.add(data.get(i).get(0).toString());
		}
	...
	private boolean equalsToLogger(String actualValue, String expected, boolean condition) {
		...
		reportManager.addScreenCapture(appiumMobileDriver.getMobileDriver());
		if (strActual.toString().trim().equals(expected.toString().trim())) {
			testResult = true;
	...
	private boolean containsLogger(String actualValue, String expected, boolean condition) {
		boolean testResult = false;
		reportManager.addScreenCapture(appiumMobileDriver.getMobileDriver());
		if (strActual.toString().trim().contains(expected.toString().trim())) {
			testResult = true;
			logSuccess(strActual, expected, condition);
		} else {
			logUnSuccess(strActual, expected, condition);
	...
	private void setStepNameDefault() {
		...
		if (noOfElements == 0) {
			...
			reportManager.takeScreenshot(appiumMobileDriver.getMobileDriver(),
					AppiumDriverBase.category + "_" + AppiumDriverBase.index);
			AppiumDriverBase.index = AppiumDriverBase.index + 1;
			reportManager.addScreenCapture(appiumMobileDriver.getMobileDriver());
